...
#-------------------------------GET/SET PLAYER DETAIL-----------------------------------------------------
player_name = sg.popup_get_text('Enter your player name', 
                    no_titlebar= True, 
...
level_and_stage_frame = [
            ...
            sg.T('Stage', 
                font=('arial', 11, 'bold'), 
                background_color=main_background_color), 
            sg.T('1', font=('arial', 11, 'bold'), 
                key='stage_label', 
                background_color=main_background_color)
            ]]


retry_frame = [[sg.T(retry, 
                    font=('arial', 11, 'bold'), 
                    key='retry_label', 
                    size=(5,1), 
                    background_color=main_background_color)
...
highscore_and_score_frame = [
                                ...
                                sg.T('0', 
                                    ...
                                    font=('arial', 11, 'bold'), 
                                    background_color=main_background_color, 
                                    key='currentscore_label')
                                ]
                            ]

game_word_frame = [[sg.T(dashed, 
...
main_layout = [[sg.T('Player ', 
                    font=('arial', 11, 'bold'), 
                    background_color=main_background_color), 
                    sg.T(player_name, font=('arial', 11, 'bold'), 
                    background_color=main_background_color,
                    key='player_name')],
               [sg.Frame('Position', 
                        layout=level_and_stage_frame, 
                        title_location='n', 
                        background_color=main_background_color), 
                    sg.Frame('Guess Left', 
                        layout=retry_frame, 
                    ...
                    sg.Frame('Scores', 
                        layout=highscore_and_score_frame, 
                        title_location='n', 
                        background_color=main_background_color, )],
               [sg.Frame('', layout=game_word_frame, 
                        title_location='n',
                        background_color='black', 
                        border_width=-1)],
               [sg.Frame('Definition/guide', 
                        layout=definition_frame,
                        background_color=main_background_color, 
                        title_location='n', 
                        element_justification='left')],
               [sg.Frame('Keys', 
                        layout=letters_frame,
                        background_color=main_background_color, 
                        title_location='n')],
               [sg.B('Top 10 Players', 
                    key='top10', 
                    button_color=('white', '#005500')), 
                sg.B('Stop Game', 
                    key='stop', 
                    button_color=('white', '#550000')),
                sg.B('Reset All', 
                    key='reset', 
                    button_color=('white', '#550000'))]]


game_window = sg.Window('Wordee Game', 
                        layout=main_layout, 
                        element_justification='center',
...
while True:
    ...
    if events == 'top10':
        sg.popup_no_buttons(formattedHighscoreList(), 
                        auto_close=True, 
                        auto_close_duration=2, 
                        background_color='#116611',
                        text_color='white', 
                        font=('arial', 14, 'bold'), 
                        no_titlebar=True,)

    if events == 'reset':
        reset_game_data = sg.popup_get_text(message="This will clear all game data.\n type 'reset' to continue")

        if reset_game_data == 'reset': 
            sg.popup_timed('resetting game ...\n You need to restart game after.', auto_close_duration=2, no_titlebar=True,)   
            db.resetUserData()
            break
    ...
    if events in alphabet:
        ...
        if not positions:
            ...
            sg.popup_no_buttons('INVALID CHOICE', 
                                auto_close=True, 
            ...
            if retry < 0:
                    sg.popup_no_buttons('NO RETRIES LEFT. GAME OVER!!!', 
                        auto_close=True, 
        ...
        if letter not in selected_letters:
            ...
            score += 150 * consecutive_correct #reward for consecutive correct guesses
